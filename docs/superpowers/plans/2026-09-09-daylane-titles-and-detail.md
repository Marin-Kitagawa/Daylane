...
- Nothing in the spec said what happens when *both* an ignore rule and a privacy keyword match the same window. Task 6 step 4 fixes the order — exclusion is evaluated against the title that will be STORED, i.e. after suppression strips it — because the reverse produces a live verdict the Task 9 recompute can never reproduce, so a row excluded by a title keyword would silently un-exclude itself on the next unrelated rule edit. Consequence, intended: title-keyword rules require title capture to be on; whole-process rules work regardless. (This paragraph originally asserted the opposite ordering; corrected during the pre-flight scan, see the R1 ruling in the ledger.)
